...
export const generateTokens = (userId: string, email: string, jti: string, role: string = 'user') => {
  const accessToken = jwt.sign({ sub: userId, email, role, type: 'access' }, ACCESS_SECRET, { expiresIn: '7d' });
  const refreshToken = jwt.sign({ sub: userId, email, jti, role, type: 'refresh' }, REFRESH_SECRET, { expiresIn: '7d' });
